...
import conf # Импортирует модуль с токеном
import Map
...
bot = telebot.TeleBot(conf.TOKEN) # Загружает бота в программу, используя токен из модуля conf, для загрузки токена достаточно создать в генеральной папке файл conf.py и в константу TOKEN загрузить токен
User = {
    "class":'',
